...
class uipress_menu_creator
{
  ...
  public function uipress_get_menus()
  {
    if (defined("DOING_AJAX") && DOING_AJAX && check_ajax_referer("uipress-menu-creator-security-nonce", "security") > 0) {
      ...
      foreach ($menus as $menu) {
        ...
        $temp["roleMode"] = get_post_meta($menu->ID, "role_mode", true);
        $temp["appliedTo"] = get_post_meta($menu->ID, "applied_to", true);
        $temp["autoUpdate"] = get_post_meta($menu->ID, "autoUpdate", true);
        $temp["availableTop"] = get_post_meta($menu->ID, "availableTop", true);
        $temp["availableSub"] = get_post_meta($menu->ID, "availableSub", true);
  ...
  public function uipress_save_menu()
  {
    if (defined("DOING_AJAX") && DOING_AJAX && check_ajax_referer("uipress-menu-creator-security-nonce", "security") > 0) {
      ...
      $tops = json_decode(stripslashes($_POST["availableTop"]));
      $subs = json_decode(stripslashes($_POST["availableSub"]));

      $tops = $this->utils->clean_ajax_input($tops);
      $subItmes = $this->utils->clean_ajax_input($subs);
      ...
      update_post_meta($themenuID, "autoUpdate", $autoUpdate);
      update_post_meta($themenuID, "subsites", $subs);
      update_post_meta($themenuID, "role_mode", $menu->roleMode);
      update_post_meta($themenuID, "applied_to", $menu->appliedTo);
      update_post_meta($themenuID, "availableTop", $tops);
      update_post_meta($themenuID, "availableSub", $subItmes);
  ...
  public function build_editor()
  {
    ...
    ?>
		...
		<div  class="uip-flex" v-if="ui.editingMode" style="height:calc(100vh - 73px - var(--uip-toolbar-height)); max-height:calc(100vh - 73px - var(--uip-toolbar-height))">
            ...
            <div class="uip-margin-bottom-m">
              <div class="uip-text-bold uip-margin-bottom-xs"><?php _e("Auto Update", "uipress"); ?></div>
              <label class="uip-switch">
                <input type="checkbox" v-model="user.currentMenu.autoUpdate">
                <span class="uip-slider"></span>
              </label>
              <p class="uip-text-muted"><?php _e("Enabling this option will allow the menu to update automatically as you add or remove plugins", "uipress"); ?></p>
            </div>
